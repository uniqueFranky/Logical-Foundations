...
Fixpoint included (s1 : bag) (s2 : bag) : bool :=
  ...
  | nil, _ => true
  | _, nil => false
  | h1 :: t1, _ => match (length s2) =? (length (remove_one h1 s2)) with
    | true => false
    | false => included t1 (remove_one h1 s2)
    end
  end.
...
Theorem add_inc_count: forall (n: nat) (l: bag), (length (add n l)) = S (length l).
Proof.
  intros n l.
  simpl.
  reflexivity.
...
Proof.
  ...
  - (* l1 = nil *)
    ...
    reflexivity.
  - (* l1 = h :: t *)
    (* IHl1: rev (t ++ l2) = rev l2 ++ rev t *)
    simpl.
    (* Goal: rev (t ++ l2) ++ [h] = rev l2 ++ rev t ++ [h] *)
    rewrite -> IHl1.
    rewrite -> app_assoc.
    reflexivity.
Qed.
...
Proof.
  ...
  - (* l = h :: t *)
    (* IHl: rev (rev t) = t *)
    simpl.
    (* Goal: rev (rev t ++ [h]) = h::t *)
    rewrite -> rev_app_distr.
    simpl.
    (* Goal: [h] ++ rev (rev t) = h::t *)
    rewrite -> IHl.
    simpl.
    reflexivity.
Qed.
...
Proof.
  intros l1 l2 l3 l4.
  assert (H1: l2 ++ (l3 ++ l4) = (l2 ++ l3) ++ l4).
  { rewrite -> app_assoc. reflexivity. }
  assert (H2: (l1 ++ l2) ++ l3 = l1 ++ l2 ++ l3).
  { rewrite -> app_assoc. reflexivity. }
  rewrite -> H1.
  rewrite -> H2.
  rewrite <- app_assoc.
...
Proof.
  ...
  - simpl. reflexivity.
  - (* l1 = h :: t *)
    (* IHl: nonzeros (t ++ l2) = (nonzeros t) ++ (nonzeros l2) *)
    simpl.
    (* Goal: nonzeors(h::(t ++ t2)) = (nonzeros (h::t)) ++ (nonzeros l2) *)
    destruct h as [| h'].
    + simpl.
      rewrite -> IHl.
      reflexivity.
    + simpl.
      (* Goal: h::nonzeros(t ++ l2) = h::((nonzeros t) ++ (nonzeros l2)) *)
      rewrite -> IHl.
      reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. reflexivity.
  - (* s = h::t *)
    (* IHs: count 0 (remove 0 t) <= count 0 t *)
    (* Goal: count 0 (remove 0 h::t) <= count 0 h::t *)
    destruct h as [| h'].
    + simpl.
      (* Goal: count 0 t <= S (count 0 t) *)
      rewrite -> leb_n_Sn.
      reflexivity.
    + (* Goal: count 0 (remove 0 t) <= count 0 t *)
    exact IHs.
Qed.
(** [] *)
...
Theorem bag_count_sum: forall (v: nat) (s1 s2: bag), (count v s1) + (count v s2) = count v (sum s1 s2).
Proof.
  intros v s1 s2.
...
Proof.
  ...
  intros H.
  simpl.
  (* Goal: find x (record y o d) = find x d *)
  rewrite -> H.
  reflexivity.
Qed.
(** [] *)
